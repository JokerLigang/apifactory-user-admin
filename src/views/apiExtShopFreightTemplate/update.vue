    ...
    <el-form :rules="rules" ref="addEditPopForm" :model="pushData" label-width="120px">
      ...
      <el-form-item label="运费设置">
        <div v-for="feeType in feeTypes" v-bind:key="feeType.type">
          <el-checkbox v-model="feeType.isChecked" @change="initCheck(feeType.type)"> {{feeType.name}}</el-checkbox>
...
<script>
  ...
  export default {
    ...
    methods: {
      initCheck(details) {
        if (vm.feeTypes[details].details.length === 0)
          vm.feeTypes[details].details.push({
            name: '默认运费',
            cityType: 0,
            cityId: 0,
            firstNumber: 1,
