  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-upload
        ...
        <el-button slot="trigger" class="filter-item" size="medium" type="primary" icon="el-icon-upload">
          上传支付证书（P12格式文件）
        </el-button>
        <el-button class="filter-item" style="margin-bottom: 10px;" size="medium" @click="handleCreate" type="success"
                   icon="el-icon-edit">修改appid/secret设置
        </el-button>
        <el-button class="filter-item" size="medium" type="primary" icon="el-icon-upload" @click="fetchQrcode">
          小程序获取二维码
        </el-button>
        <el-button class="filter-item" style="margin-bottom: 10px;" size="medium" @click="delData" type="danger"
                   icon="el-icon-delete">删除当前配置
        </el-button>
      </el-upload>

    </div>
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="Loading" :show-header="false" border fit
              highlight-current-row empty-text="暂无数据">
      <el-table-column prop="name" label="name"></el-table-column>
      <el-table-column prop="val" label="val"></el-table-column>
    </el-table>
    <h3>Accesstoken 信息</h3>
    <el-table :data="listResult" v-loading.body="listLoading" element-loading-text="Loading" :show-header="false" border
              fit highlight-current-row empty-text="暂无数据">
      <el-table-column prop="name" label="name"></el-table-column>
      <el-table-column prop="val" label="val"></el-table-column>
    </el-table>

    <el-dialog title="配置微信appid/secret信息" :visible.sync="pushData.dialogFormVisible" :close-on-click-modal="false"
    ...
    <el-dialog title="小程序获取二维码" :visible.sync="pushData2.dialogFormVisible" :close-on-click-modal="false"
               :close-on-press-escape="false">
      <el-form ref="addEditPopForm2" :model="pushData2" label-position="left" label-width="160px">
        <el-form-item label="页面路径" prop="content">
          <el-input v-model="pushData2.content" type="text" clearable
                    @keyup.enter.native="handleCreateSave2"></el-input>
        </el-form-item>
        <el-form-item label="二维码的宽度" prop="width">
          <el-input v-model.number="pushData2.width" type="text" clearable
                    @keyup.enter.native="handleCreateSave2"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
    ...
    methods: {
      ...
      fetchData() {
        ...
        infoData().then(response => {
          if (response.code == 0) {
            ...
            this.pushData.id = response.data.id
            ...
            this.listResult.push({name: "添加时间", val: response.data.addTime})
            this.listResult.push({name: "更新时间", val: response.data.updateTime})
            this.listResult.push({name: "刷新时间", val: response.data.tokenUpdateTime})
      ...
      delData() {
        this.$confirm('删除无法恢复, 是否继续?', '提示', {
          ...
          delData(this.pushData.id).then(res => {
            Message({
              message: '删除成功',
              type: 'success',
              duration: 1 * 1000,
